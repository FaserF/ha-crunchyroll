...
def main():
    ...
    repo = os.environ.get("REPO", "faserf/ha-crunchyroll").lower()
    ...
    for path in ["custom_components/crunchyroll/manifest.json", "pyproject.toml"]:
        try:
    ...
    for f in changed_files:
        if f.startswith("custom_components/crunchyroll/translations/"):
            translation_count += 1
    ...
    body_parts = [
        f"# Crunchyroll {version}  {channel_badge}",
        "",
        prerelease_note,
        "## 📋 What's Changed",
        "",
